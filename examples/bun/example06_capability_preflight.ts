...
function printUsage(): void {
  console.log("Usage: bun run examples/bun/example06_capability_preflight.ts [--dll PATH] [--backend all|dx11|dx12] [--compare-active dx11|dx12]");
}

function selectBackends(name: string): readonly (typeof BACKENDS)[number][] {
...
function parseCompareBackend(name: string): (typeof BACKENDS)[number] | null {
  if (name === "") {
    return null;
  }
  const backendId = parseBackend(name);
...
function printLegacyComparison(dxbridge: DXBridgeLibrary, backend: (typeof BACKENDS)[number]): void {
...
function printBackend(dxbridge: DXBridgeLibrary, backend: (typeof BACKENDS)[number]): void {
  ...
  for (let index = 0; index < count.valueU32; index += 1) {
    ...
    console.log(`  adapter ${index}: software=${software.valueU32 !== 0} maxFL=${formatFeatureLevel(featureLevel.valueU32)}`);
  }
}
